...
class ESMapDB {
  ...
  constructor(options, callback) {
    ...
    if(isPersistentSelected) {
      ...
      self.database = new Level(location, encoding);
      self.database.open((err) => {
        if(err) {
          ...
          if(self.isInMemory) {
            syncToMemory(self.database, self.inMemoryMap, (err) => {
              if(typeof callback === 'function') {
                return callback(err || NO_ERRORS_VALUE, self);
              }
            });
          }
 ...
 */
async function syncToMemory(database, inMemoryMap, callback) {
  try {
